...
def get_hh(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    tmp = [k for k, v in data.items() if len(v['responses'])>2]
    return data


def get_imdb_positive():
    """
       {
           'prompt1': {
               'responses': List[str],
    ...
    df = df[(~df['good_response'].isna()) & (~df['bad_response'].isna())]
    num_responses = 4
    num_pairs = 6
    data = dict()
    prompts = []
    ...
    for i in tqdm.tqdm(range(0, df.shape[0], num_pairs)):
        batch = df.iloc[i:i+num_pairs]
        prompt = batch['prompt'].unique()
        assert len(prompt) == 1
        prompt = prompt[0]
        prompt_score = dict()
        counter = 0
        for ind, row in batch.iterrows():
            prompt_score[row['good_response']] = row['good_response_score']
            prompt_score[row['bad_response']] = row['bad_response_score']
            counter += 1
        all_responses = list(prompt_score.keys())
        total_score.extend(list(prompt_score.values()))
        # if len(all_responses) != num_responses:
        #     __import__('pdb').set_trace()
        #     print()
        # assert len(all_responses) == num_responses

        all_pairs = list(itertools.combinations(range(len(all_responses)), 2))
        all_pairs = [(pair[0], pair[1]) if prompt_score[all_responses[pair[0]]] > prompt_score[all_responses[pair[1]]] else (pair[1], pair[0]) for pair in all_pairs]
        score_pairs = [(prompt_score[all_responses[pair[0]]], prompt_score[all_responses[pair[1]]]) if prompt_score[all_responses[pair[0]]] > prompt_score[all_responses[pair[1]]] else (prompt_score[all_responses[pair[1]]], prompt_score[all_responses[pair[0]]]) for pair in all_pairs]
        # if i> 100*6:
        #     __import__('pdb').set_trace()
        #     print()
    ...
    print(f'Dataset size: {len(data)}, mean_score={np.mean(total_score)}')
    # __import__('pdb').set_trace()
    return data
